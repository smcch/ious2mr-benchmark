...
class EMA(Optimizer):
    def __init__(self, opt, ema_decay):
        ...
        # Torch >= 2.0 reads these attributes inside Optimizer.state_dict() /
        # load_state_dict() even if we skip the parent __init__. Initialize
        # them to empty OrderedDicts so the base-class logic becomes a no-op.
        ...
        self._patch_step_function = lambda *a, **kw: None
        self.defaults = getattr(opt, 'defaults', {})
