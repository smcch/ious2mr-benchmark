...
def train_syndiff(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            ...
            # NOTE: original repo had torch.autograd.set_detect_anomaly(True)
            # here -- that slows training by ~3x and is a debugging tool, not
            # needed for production training.
            ...
            if getattr(args, 'max_iters_per_epoch', 0) and iteration + 1 >= args.max_iters_per_epoch:
                break
        ...
        # Validation is slow (full diffusion sampling over the whole val set
        # twice). Skip unless we're on a val epoch.
        val_every = max(1, getattr(args, 'val_every', 1))
        val_max_iters = getattr(args, 'val_max_iters', 0)
        if rank == 0 and (epoch % val_every == 0):
            for iteration, (x_val , y_val) in enumerate(data_loader_val):
                if val_max_iters and iteration >= val_max_iters:
                    break
            ...
            print(f'[val] epoch {epoch} PSNR(US->T2)={np.nanmean(val_psnr_values[0,epoch,:]):.3f} '
                  f'PSNR(T2->US)={np.nanmean(val_psnr_values[1,epoch,:]):.3f}')
...
if __name__ == '__main__':
    ...
    # --- our additions for practical training on small GPUs ---
    parser.add_argument('--val_every', type=int, default=1,
                        help='run validation only every N epochs (default 1 = every epoch)')
    parser.add_argument('--val_max_iters', type=int, default=0,
                        help='cap number of val iterations per epoch (0 = full val set)')
    parser.add_argument('--max_iters_per_epoch', type=int, default=0,
                        help='cap training iterations per epoch (0 = full epoch)')
