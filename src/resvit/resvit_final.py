"""
ResViT Faithful — Final Unified Implementation
================================================
Dalmaz et al., 2022: Aggregated Residual Transformers for Multi-Modal
Medical Image Synthesis

Faithful to the original paper:
  - Swin Transformer with shifted windows (W-MSA / SW-MSA alternation)
  - ART blocks: parallel CNN + Transformer → concat + 1x1 aggregation
  - Independent transformer weights per ART block (not shared)
  - Encoder: ReflPad7-Conv7-IN-ReLU + 2× downsample
  - Decoder: 2× ConvTranspose + ReflPad-Conv7-Tanh
  - Two-phase training: Phase 1 CNN-only, Phase 2 full ART
  - PatchGAN discriminator with LSGAN + feature matching

Variants:    2d | 2.5d | 2d_3d_refine | full_3d
Targets:     t2 | flair | t2_flair (masked loss for partial availability)
Inference:   Axial-only (triplanar is opt-in, NOT default for anisotropic US)
Split:       Fixed from subject_split.json (consistent with MMHVAE experiments)
GPU:         Auto-profiled (RTX 3090 24GB / RTX 4080 SUPER 16GB / 8-12GB)

Usage:
  python resvit_final.py --variant 2d --target t2
  python resvit_final.py --variant 2d_3d_refine --target t2_flair
  python resvit_final.py --variant full_3d --target t2 --phase1_epochs 60
"""
...
class Config:
    ...
    # --- Architecture ---
    arch_variant = "2d"          # 2d | 2.5d | 2d_3d_refine | full_3d
    ...
    @property
    def run_name(self):
        return f"ResViT-{self.arch_variant}-{self.target_mode}"

    def _dir(self, sub):
        d = os.path.join(self.output_root, self.run_name, sub)
...
class Gen3D(nn.Module):
    def __init__(self, ic=1, oc=1, ngf=48, nart=9, tpos=(4,5),
                 nh=8, slp=2, ws=4, mr=4., dr=0., ckpt=False):
        super().__init__()
        bn = ngf*4
        self.enc = nn.Sequential(
...
def build_all(cfg_obj, dev):
    ...
    if v in ("2d","2.5d","2d_3d_refine"):
        G = Gen2D(ic, oc, cfg_obj.ngf, cfg_obj.n_art_blocks, cfg_obj.transformer_positions,
                  ...
                  cfg_obj.mlp_ratio, cfg_obj.dropout).to(dev)
        ...
        G = Gen3D(1, oc, cfg_obj.ngf, cfg_obj.n_art_blocks, cfg_obj.transformer_positions,
                  ...
                  cfg_obj.mlp_ratio, cfg_obj.dropout, cfg_obj.grad_ckpt_3d).to(dev)
...
def train_ep(G, D, ld, oG, oD, sc, dev, ep, acc, la, ll, lf):
    ...
    for i, (inp, tar, cm) in enumerate(ld):
        ...
        # ---- Single G forward (reused for D & G updates) ----
        with _amp_ctx(dev):
            fk_raw = G(inp)
        ...
        r["l1"]+=ll_.item(); r["adv"]+=la_.item(); r["fm"]+=lf_.item(); n+=1
        if (i+1)%50==0:
            print(f"  E{ep} [{i+1}/{len(ld)}] L1:{ll_.item():.4f} Adv:{la_.item():.4f} "
                  f"FM:{lf_.item():.4f} D:{ld_.item()*acc:.4f}", flush=True)
...
def val_ep(G, ld, dev):
    ...
    for inp, tar, cm in ld:
        ...
        cm  = cm.to(dev, non_blocking=True)
        with _amp_ctx(dev):
            s += masked_l1(Gv(inp), tar, cm).item()
        n += 1
    return s / max(n,1)
...
def run_phase(G, D, tl, vl, vs, dev, sc, nep, lrg, lrd, name, cfg_obj, resume=True):
    ...
    for ep in range(start_ep, nep+1):
        t0 = time.time()
        st = train_ep(G, D, tl, oG, oD, sc, dev, ep, acc, cfg_obj.lambda_adv, cfg_obj.lambda_l1, cfg_obj.lambda_fm)
        sG.step(); sD.step()
        print(f"  [{name}] E{ep}/{nep} G:{st['g']:.4f} D:{st['d']:.4f} L1:{st['l1']:.4f} | {time.time()-t0:.1f}s")
...
def main():
    pa = argparse.ArgumentParser("ResViT Faithful")
