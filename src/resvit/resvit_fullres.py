...
# -*- coding: utf-8 -*-
"""
resvit_fullres.py  —  ResViT 2.5d  (target T2)  retrained on the NEW full-resolution
dataset: the "cropped" representation of dataset-registration-baseline (native
~0.13x0.13x0.5mm grid, cropped to the US-FOV cone bbox, MR masked to the cone; the US is
the original, just cropped — never resampled). MR registered onto the US with the
Learn2Reg MIND-SSC baseline.
...
Usage:
  ...
  python resvit_fullres.py                       # 2.5d / t2, defaults from resvit_final.py
  python resvit_fullres.py --phase1_epochs 100 --phase2_epochs 100   # pass-through args ok
  python resvit_fullres.py --no_resume           # train from scratch (ignore checkpoints)
"""
